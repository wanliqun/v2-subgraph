...
export function createLiquiditySnapshot(position: LiquidityPosition, event: ethereum.Event): void {
  ...
  if (token0.derivedETH === null || token1.derivedETH === null) return
  ...
  snapshot.token0PriceUSD = token0DerivedETH.times(bundle.ethPrice);
  snapshot.token1PriceUSD = token1DerivedETH.times(bundle.ethPrice);
