...
export function findEthPerToken(token: Token): BigDecimal {
  ...
  for (let i = 0; i < WHITELIST.length; ++i) {
    ...
    if (pairAddress.toHexString() != ADDRESS_ZERO) {
      ...
      if (pair === null) return ONE_BD;
      if (pair.token0 == token.id && pair.reserveETH.gt(MINIMUM_LIQUIDITY_THRESHOLD_ETH)) {
        let token1 = Token.load(pair.token1)
        if (token1 === null) return ONE_BD;
        return pair.token1Price.times(token1.derivedETH as BigDecimal) // return token1 per our token * Eth per token 1
      }
      if (pair.token1 == token.id && pair.reserveETH.gt(MINIMUM_LIQUIDITY_THRESHOLD_ETH)) {
        let token0 = Token.load(pair.token0)
        if (token0 === null) return ONE_BD;
...
export function getTrackedLiquidityUSD(
  ...
  let bundle = Bundle.load('1')
  if (bundle === null) return ZERO_BD
  if (token0 === null || token1 === null) return ZERO_BD
  if (token0.derivedETH === null || token1.derivedETH === null) return ZERO_BD
